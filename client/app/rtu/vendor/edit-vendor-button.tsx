...
import { Edit, Trash2, Globe, Link2 } from 'lucide-react';
...
/** Normalize contacts from possibly-legacy vendor data */
function normalizeContacts(vendor: RTUVendor) {
  if (Array.isArray(vendor.contacts) && vendor.contacts.length > 0) {
  ...
  // Legacy fallback: vendor had flat contact/phone fields
  if (vendor.contact) {
    return [
      {
        name: vendor.contact,
        phone:
          Array.isArray(vendor.phone) && vendor.phone[0] ? vendor.phone[0] : '',
        email: ''
      }
    ];
  }
...
export function EditVendorButton({ vendor }: { vendor: RTUVendor }) {
  ...
  const form = useForm<any>({
    ...
    defaultValues: {
      ...
      contacts: normalizeContacts(vendor),
      links: vendor.links || [],
  ...
  const onSubmit = async (values: any) => {
    try {
      ...
      await api.put(`/rtu/vendor/${vendor._id}`, payload);
      ...
      toast.error(error?.response?.data?.message || 'Gagal update Vendor');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          className="h-6 w-6 p-0 hover:bg-blue-50 text-blue-500"
          variant="outline"
          size="icon"
          icon={Edit}
        />
        ...
        >
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              />
              <FormInput form={form} name="name" label="Nama Vendor *" />
            </div>
                    ...
                    return (
                      <div key={field.id} className="space-y-1.5">
                        <div className="flex gap-2 items-center">
                          {/* Platform Icon Badge */}
                          <div className="flex items-center justify-center h-9 w-9 rounded-sm bg-gray-50 border border-gray-200 shrink-0">
            ...
            <FormInput form={form} name="notes" label="Catatan" rows={3} />
          ...
          <div className="p-4 px-6 bg-gray-50 border-t border-gray-100 flex justify-end space-x-2 rounded-b-sm shrink-0">
            <Button
              type="button"
              variant="outline"
