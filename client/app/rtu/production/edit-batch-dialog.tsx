...
import { Pencil, LayoutPanelTop, Info, Package, Boxes } from 'lucide-react';
...
export function EditBatchDialog({
  ...
  const defaultMaterialUsages = useMemo(() => {
    return (batch.materialUsages || []).map((mu) => ({
      recipeIngredientId: mu.recipeIngredientId || '',
  ...
  const form = useForm<BatchFormValues>({
    ...
    defaultValues: {
      ...
      plannedDate: batch.plannedDate
        ? dayjs(batch.plannedDate).format('YYYY-MM-DD')
        : dayjs().format('YYYY-MM-DD'),
      outletId: batch.outletId || '',
      actualQty: batch.actualQty || 1,
      notes: batch.notes || '',
      materialUsages: defaultMaterialUsages
    }
  });
  ...
  }, [open, batch, reset, defaultMaterialUsages]);
  ...
  const activeRecipeVersion = useMemo(() => {
    if (selectedProductId === batch.product?._id && batch.recipeVersion) {
      return batch.recipeVersion;
    }
    return selectedProduct?.recipe?.versions?.find(
      (v: any) => v.status === 'active'
    );
  }, [selectedProduct, selectedProductId, batch]);
  ...
  const onSubmit = async (data: BatchFormValues) => {
    ...
    const activeVersion = activeRecipeVersion;
    if (!activeVersion) {
    ...
    try {
      await api.put(`/rtu/batch/${batch._id}`, {
        ...data,
      ...
      toast.success('Production Batch edited successfully');
      ...
      toast.error(error.response?.data?.message || 'Failed to edit batch');
    } finally {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          ...
          className="h-7 w-7 p-0 hover:bg-orange-50 hover:text-orange-600 text-orange-500 cursor-pointer rounded-sm"
      ...
      <DialogContent
        className="max-w-3xl max-h-[90vh] rounded-sm border-none shadow-2xl flex flex-col overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <DialogHeader>
                ...
                <div className="space-y-2 pt-1">
                  {activeRecipeVersion?.ingredients?.map((ing: any) => {
                    ...
                    const selectedMatId =
                      materialUsages.find(
                        (m: any) =>
                          (m.recipeIngredientId &&
                            m.recipeIngredientId === ing._id) ||
                          m.materialId === ing.materialId ||
                          (ing.alternativeMaterialIds &&
                            Array.isArray(ing.alternativeMaterialIds) &&
                            ing.alternativeMaterialIds.includes(m.materialId))
