...
import { useGetRTUMaterialLots } from '@/lib/hooks/queries/rtu-material';

interface MaterialLotsDialogProps {
  material: {
    _id: string;
    name: string;
    code: string;
...
export function MaterialLotsDialog({
  material,
  outletId
}: MaterialLotsDialogProps) {
  const [open, setOpen] = useState(false);
  const { data, isLoading } = useGetRTUMaterialLots(material._id, outletId);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          variant="outline"
          className="rounded-sm font-bold text-[10px] px-2 h-6 gap-1 border-emerald-200 hover:bg-emerald-50 text-emerald-700 hover:text-emerald-800"
        >
          ...
          <div className="flex justify-between items-start">
            <div>
              <DialogTitle className="text-2xl font-bold tracking-tight text-gray-900 flex items-center gap-2">
                ...
                Breakdown Lot FIFO - {material.name}
              </DialogTitle>
          ...
          {isLoading ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-gray-400 text-xs mt-1">
                Stok material ini kosong atau belum memiliki catatan lot di
                outlet terpilih.
              </p>
            </div>
          ) : (
                      ...
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <TooltipProvider delayDuration={100}>
                          <Tooltip>
                            ...
                            >
                              HPP (Harga Pokok Penjualan) ini didapatkan murni
                              dari{' '}
                              <span className="font-bold text-emerald-400">
                                harga beli bahan baku
                              </span>{' '}
                              pada dokumen penerimaan (GRN).
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </div>
                      ...
                      <Fragment key={lot._id}>
                        ...
                        >
                          ...
                          <TableCell className="text-xs font-bold py-3.5 text-right">
                            ...
                            >
                              {lot.qtyRemaining} {material.unit}
                            </span>
                          </TableCell>
                          <TableCell className="text-xs font-semibold py-3.5 text-right">
                          ...
                          <TableCell className="text-xs py-3.5 text-center pr-4">
                            <div className="flex items-center justify-center gap-1.5">
                              ...
                              >
                                ...
                                {isDeficit ? 'DEFICIT' : lot.refType || 'GRN'}
                              </Badge>
                            </div>
                          </TableCell>
                        </TableRow>
                        {lot.movements && lot.movements.length > 0 && (
                          <TableRow className="bg-slate-50/30 border-b border-gray-100 hover:bg-slate-50/30">
                            <TableCell colSpan={6} className="p-0 border-none">
                              <div className="bg-slate-100/50 py-2.5 px-6 border-l-4 border-emerald-400 ml-4 my-1.5 text-xs rounded-r-md">
